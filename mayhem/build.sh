#!/usr/bin/env bash
#
# mayhem/build.sh — build golang-appengine's remote_api RPC wire parser as a
...
#
# HARNESS STAGING (netnew §6 Go): go-118-fuzz-build's package loader dies on a dir
# that mixes internal(package X)+external(package X_test) test files or testing.B
# benchmarks, both of which appear throughout the upstream tree. The remote_api
# parser + proto.Unmarshal are EXPORTED, so the harness is its own standalone
# package that imports them via the public API — we stage it (plus the KAT export)
# into a fresh, single-package, leading-underscore dir (skipped by `go build/test
...
TARGET="fuzz_api"
STAGE="$SRC/_mayhem_harness/apiparse"

# ── Stage the standalone harness package (harness + KAT export) ────────────────
rm -rf "$STAGE"
mkdir -p "$STAGE"
cp "$SRC/mayhem/harness_apiparse.go.src" "$STAGE/harness_apiparse.go"
...
# ── Build the libFuzzer archive from the staged single-package harness ─────────
mkdir -p "$SRC/mayhem-build"
echo "=== go-118-fuzz-build $TARGET (func FuzzResponse) ==="
go-118-fuzz-build -func FuzzResponse -o "$SRC/mayhem-build/$TARGET.a" ./_mayhem_harness/apiparse
...
go build -o /mayhem/appengine_kat ./mayhem/kat
file /mayhem/appengine_kat | grep -q 'dynamically linked' \
  || { echo "FATAL: /mayhem/appengine_kat is not dynamically linked — oracle would be reward-hackable"; exit 1; }
echo "built /mayhem/appengine_kat (dynamically linked)"
